...
module.exports = {
    ...
    addCart: async(req, res) => {
        try {
            ...
            return res.status(200).json({ message: 'Yeay, your game has been add to cart!', yourGame: game });
        } catch (error) {
            console.log('Error in', error);
            res.status(400).json({ message: 'Please check again your code!' });
        }
    ...
    addBilling: async(req, res) => {
        try {
            ...
            const postData = await Billing.create({
                gameId,
                userId,
                paymentId,
                status: 'unpaid'
            });
            return res.status(200).json({ message: 'Success, this your billing! ', postData });
        } catch (error) {
            console.log('Error in', error);
            res.status(400).json({ message: 'Please check again your code!' });
        }
    },
    showBilling: async(req, res) => {
        try {
            const { id } = req.params;
            const billing = await Billing.findOne({ _id: id }).populate('gameId').populate('userId').populate('paymentId');
            res.status(200).json({ billing });
        } catch (error) {
            console.log('Error in', error);
            res.status(400).json({ message: 'Please check again your code!' });
    ...
    /* =========  Library ========= */
    userLibrary: async(req, res) => {
        try {
            const userId = req.params.id;
            console.log(userId);
            const library = await Library.find({ userId }).populate('userId').populate('gameId');
    ...
    addLibrary: async(req, res) => {
        try {
            ...
            const postLibrary = await Library.create({
                gameId,
                userId,
            });
            res.status(200).json({ message: 'Success, your library has been created!', postLibrary });
        } catch (error) {
            console.log('Error in', error);
            res.status(400).json({ message: 'Please check again your code!' });
        }
